...
class CIFARParser(Parser):
    ...
    def parse_annotation(self, idx: int, img: np.array, file_name: str, labels: List[str]) -> ImageAnnotationFile:
    ...
    def parse_batch(self, batch_path: Path, labels: List[str]):
        with (batch_path).open('rb') as f:
            ...
            ann_stage = map(lambda x: self.parse_annotation(*x, labels), zip(labels_ids, images, file_names))
